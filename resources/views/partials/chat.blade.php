...
<script type="text/javascript">
  ...
  $(document).ready(function(){
  	...
  	$('html').on('click','*[bom-click]',function(){
  		...
  		var title = _.where(ManageChat.listfriend, {usergroup_id: usergroup_id})[0].fullname;
  		$.ajax({
  			url : UrlBase+'chat/people/' + usergroup_id,
  			method : 'GET',
  		}).done(function(response){
  			var messages = JSON.parse(response); 
  			var fullname_user = '{{Auth::user()->fullname}}'; 
  			var wrapdataObj = Object.create(wrapdata);
  			wrapdataObj.init(user_me,usergroup_id,messages,title);
  	...
  	$('#area-chat').on('click','.close-chat',function(){
  		...
  		//ManageChat.findAreaPos(dataPos).stopLoop();
